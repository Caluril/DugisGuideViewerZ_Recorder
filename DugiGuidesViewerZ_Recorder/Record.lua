...
function Record:Initialize()
	...
	local function EvaluateQuestRecordLimit()
		...
		local h = #DugisGuideViewer.chardb.QuestRecordTable * 13 + 150
		DugisRecordFrame.scroll.child:SetHeight(h)	
		
		DugisRecordFrame.scroll.bar:SetMinMaxValues(0, h);
	...
	local orig = C_QuestLog.AbandonQuest 
	...
	function Record:Load()
		...
		function MOD:SaveBuffersAsOld()
			oldquests = currentquests
			oldboards = currentboards
			oldcompletes = currentcompletes
		end
		...
		function MOD:UpdateRecord()
			...
			for i=1, C_QuestLog.GetNumQuestLogEntries() do
				...
				if qid and qid ~= 0 then
					...
					currentcompletes[qid] = isComplete == true and info.title or nil
				end
			end
			...
			for qid in pairs(currentquests) do
				if not oldquests[qid] then
					...
					if auto then
						...
						if QuestUtils_IsQuestWorldQuest(qid) == false then 
